...
function initializeSchema(db) {
  ...
  // Add is_repeat column to period_assignments if it doesn't exist yet
  try {
    db.exec('ALTER TABLE period_assignments ADD COLUMN is_repeat INTEGER NOT NULL DEFAULT 0');
  } catch {}

  // Add rotation_position column to members if it doesn't exist yet
  try {
    db.exec('ALTER TABLE members ADD COLUMN rotation_position INTEGER');
  } catch {}

  // Initialize rotation positions for any members that don't have one yet
  // (ordered by line number, then name — lower = assigned sooner)
  const unranked = db.prepare(`
    SELECT id FROM members
    WHERE rotation_position IS NULL
    ORDER BY
      CASE WHEN line_number IS NULL OR line_number = '' THEN 1 ELSE 0 END,
  ...
  if (unranked.length > 0) {
    ...
    const upd = db.prepare('UPDATE members SET rotation_position = ? WHERE id = ?');
    let pos = maxPos;
    for (const m of unranked) upd.run(++pos, m.id);
  }
  ...
  for (const [k, v] of settingsDefaults) upsertSetting.run(k, v);
